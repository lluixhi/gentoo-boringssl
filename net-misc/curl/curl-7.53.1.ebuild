...
IUSE+=" curl_ssl_axtls curl_ssl_boringssl curl_ssl_gnutls curl_ssl_libressl curl_ssl_mbedtls curl_ssl_nss +curl_ssl_openssl curl_ssl_polarssl curl_ssl_winssl"
IUSE+=" elibc_Winnt"
...
RDEPEND="ldap? ( net-nds/openldap[${MULTILIB_USEDEP}] )
	ssl? (
		...
		curl_ssl_boringssl? (
			dev-libs/boringssl:0=[${MULTILIB_USEDEP}]
		)
		curl_ssl_gnutls? (
...
multilib_src_configure() {
	...
	if use ssl ; then
		if use curl_ssl_axtls; then
			...
			myconf+=( --with-axtls )
		elif use curl_ssl_boringssl; then
			einfo "SSL provided by BoringSSL"
